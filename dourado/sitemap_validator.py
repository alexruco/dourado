...
def fetch_sitemap(url):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            log_error(f"Failed to fetch sitemap: Status code {response.status_code}")
            return None
    except requests.RequestException as e:
        log_error(f"Failed to fetch sitemap: {e}")
        return None

def validate_sitemap(sitemap_content):
    """
    ...
    try:
        ...
        log_success("Sitemap is valid XML.")
        return True
    except ET.ParseError as e:
        log_error(f"Sitemap is invalid XML: {e}")
        return False
...
if __name__ == "__main__":
    ...
    if sitemap_content:
        log_success(f"Fetched sitemap content:\n{sitemap_content[:200]}...")  # Log only the first 200 characters
        is_valid = validate_sitemap(sitemap_content)
        if is_valid:
